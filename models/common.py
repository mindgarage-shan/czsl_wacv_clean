...
# from .multihead_attention import MultiHeadAttention as gumbel_head
...
class TransformerDecoderLayerOptimal(nn.Module):
    def __init__(self, d_model, nhead=4,
        ...
        self.norm_memory= nn.LayerNorm(d_model,eps=layer_norm_eps)
        self.multihead_attn = nn.MultiheadAttention(d_model, nhead, dropout=0.1)
        # self.multihead_attn = gumbel_head(d_model,4)
    ...
    def forward(self, tgt, memory, tgt_mask= None,
        ...
        tgt = self.norm1(tgt)
        # memory= self.norm_memory(self.dropout3(memory))

        # print(tgt.shape)
        # print(memory.shape)
        tgt2,_ = self.multihead_attn(tgt,memory, memory)
        # instead of applying simple dropout here. we will apply attention dropout here.
        tgt=tgt+self.dropout(tgt2)
        tgt=self.norm2(tgt)
        tgt=tgt.transpose(0,1)
        # output=output.transpose(0,1)
        return None,None,tgt
class AttentionDropout(nn.Module):
    def __init__(self, p: float = 0.4, d_model=768,nhead=4):
        super(AttentionDropout, self).__init__()
        if p < 0 or p > 1:
            raise ValueError("dropout probability has to be between 0 and 1, " "but got {}".format(p))
        self.p = p

        layer_decode = TransformerDecoderLayerOptimal(d_model=d_model,nhead=nhead)
    ...
    def forward_old(self, x):
        ...
        feat_dim=x.shape[-1]
        a=x.permute(1,0,2)
        attention=self.decoder(a,a).permute(1,0,2)
        dropout_attention=1-attention
        dropout_attention=dropout_attention.view(bs,-1)
        topk=torch.topk(dropout_attention,int(self.p*dropout_attention.shape[1]))
        activations=torch.ones(dropout_attention.shape)
        activations=activations.scatter(1,topk.indices, 0)
        activations=activations.reshape(bs,seq_dim,seq_dim)
        X=torch.sparse.mm(X,activations.transpose(-2,-1))
        return X
    def forward(self, x1,x2):
        ...
        # a=x.permute(1,0,2)
        # print(x.shape)
        output,attention,_=self.decoder(x1,x2)
...
class Cross_Classifier(nn.Module):
    ...
    Inputs
        ...
        bias: Bool, Use bias
    '''
    def __init__(self, feat_dim,output_dim, attention_dropout=False):
        super(Cross_Classifier, self).__init__()
        self.attr_clf_middle = MLP(feat_dim, feat_dim, num_layers=1, relu = True, dropout=True, norm=True)
        self.obj_clf_middle = MLP(feat_dim, feat_dim, num_layers=1, relu = True, dropout=True, norm=True)
        self.attr_clf = MLP(feat_dim+768, 1, num_layers=3, relu = True, dropout=True, norm=True)
        self.obj_clf = MLP(feat_dim+768,1, num_layers=3, relu = True, dropout=True, norm=True)
        self.attention_dropout=attention_dropout
        if attention_dropout:
            self.attention_dropout_obj=AttentionDropout(p=0.4,d_model=feat_dim)
            self.attention_dropout_attr=AttentionDropout(p=0.4,d_model=feat_dim)
        else:
            layer_decode_attr = TransformerDecoderLayerOptimal(d_model=feat_dim)
            layer_decode_obj = TransformerDecoderLayerOptimal(d_model=feat_dim)
    ...
    def forward(self,attr_query,obj_query,cls_token):
        # attr_o=self.attr_clf_middle(attr)
        # obj_o=self.obj_clf_middle(obj)
        # attr_o=torch.cat([attr_query,cls_token.expand([attr_query.shape[0],-1,-1])],dim=-1)
        # obj_o=torch.cat([obj_query,cls_token.expand([obj_query.shape[0],-1,-1])],dim=-1)
        attr_o=attr_query.transpose(0,1)
        obj_o=obj_query.transpose(0,1)
        # print(attr_o.shape)

        if self.attention_dropout:
            obj_o=self.attention_dropout_obj(obj_o,attr_o)
            attr_o=self.attention_dropout_attr(attr_o,obj_o)
        else:
            _,_,attr_o=self.decoder_attr(attr_o,obj_o)
            _,_,obj_o=self.decoder_obj(obj_o,attr_o)
        attr_o=torch.cat([attr_o,cls_token.expand([-1,attr_o.shape[1],-1])],dim=-1)
        obj_o=torch.cat([obj_o,cls_token.expand([-1,obj_o.shape[1],-1])],dim=-1)
        attr_o=self.attr_clf(attr_o).squeeze(-1)
        obj_o=self.obj_clf(obj_o).squeeze(-1)
        return attr_o,obj_o
...
class MLP(nn.Module):
    ...
    Inputs
        ...
        bias: Bool, Use bias
    '''
    def __init__(self, inp_dim, out_dim, num_layers = 1, relu = True, bias = True, dropout = False, norm = False, layers = [],attention_dropout=True,dropout_prob=0.5):
        ...
        for layer in range(num_layers - 1):
            ...
            if dropout:
                mod.append(nn.Dropout(p = dropout_prob))

            #     if attention_dropout and i==0:
            #         mod.append(AttentionDropout(p=0.4,d_model=outgoing))
